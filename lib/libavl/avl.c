...
int
eob_panic(const char *file, const char *func, int line, const char *fmt, ...) {
	const char *newfile;
	char msg[MAXMSGLEN];
	va_list ap;
...
module_init(eavl_init);
module_exit(eavl_fini);

MODULE_DESCRIPTION("Generic AVL tree implementation");
MODULE_AUTHOR("ZFS Team");
MODULE_LICENSE("GPL");
